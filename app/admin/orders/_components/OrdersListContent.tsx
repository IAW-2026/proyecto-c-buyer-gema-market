...
export async function OrdersListContent({ searchParams }: { searchParams: SearchParams }) {
  await requireAdmin();
  ...
  const [ordenes, total] = await Promise.all([
    getOrdenesPaginatedAdmin({ skip: (page - 1) * ADMIN_ORDERS_PAGE_SIZE, take: ADMIN_ORDERS_PAGE_SIZE, search }),
    countOrdenesAdmin(search),
  ]);

  const productIds = ordenes.map((o) => o.productId);
  const { products } = productIds.length > 0 ? await getProductsBatch(productIds) : { products: [] };
  const productMap = Object.fromEntries(products.map((p) => [p.product_id, p.title]));
  ...
  return (
    <>
      <SectionTitle eyebrow={`${total} orden${total !== 1 ? "es" : ""}`}>Órdenes</SectionTitle>
      <SearchBar placeholder="Buscar por nombre o email…" />
      {total === 0 ? (
        <EmptyState
          icon="box"
          title="Sin órdenes"
          body={search ? "No se encontraron órdenes para ese comprador." : "Todavía no hay órdenes registradas."}
        />
      ) : (
        <>
          <OrdersTable rows={rows} />
          <div className="flex justify-center mt-6">
            <Pagination totalPages={Math.max(1, Math.ceil(total / ADMIN_ORDERS_PAGE_SIZE))} />
          </div>
        </>
      )}
    </>
